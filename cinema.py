...
import math
mydb = mysql.connector.connect(host="localhost",user = "root", password = "15921039Ss@", database = "bookmymovie")
...
class Cinema:
                    ...
                    if i ==1 and j == 0:
                        ...
                        while(True):
                            if count <= seats:
                                ...
                                sql = 'select * from customer_details where seat_no = %s'
                                val = (seatno,)
    ...
    def buy_ticket(self, row, seats):
        ...
        if brow <= row and bcol <= seats:
            ...
            if ch == "yes":
                ...
                sql = "insert into customer_details(Name,Gender,Age,phone_number,seat_no,ticket_price) values (%s,%s,%s,%s,%s,%s);"
                val = (name, gender, age, phone, seat_no, ticket_price)
                mycursor.execute(sql, val)
                mydb.commit()
                mycursor.execute('select * from customer_details where Name = name')
    ...
    def show_statistics(self,row,seats):
        ...
        sql = "select sum(ticket_price) from customer_details where seat_no <= %s"
        val = (seat_data,)
        mycursor.execute(sql,val)
        res = mycursor.fetchone()
        for price in res:
    ...
    def show_booked_ticket(self):
        ...
        sql = "select Name,Gender,Age,ticket_price,phone_number from customer_details where seat_no = %s;"
        val = (sno,)
        mycursor.execute(sql, val)
        res = mycursor.fetchone()
        if res:
